...
```{r cargar_datos}
# Cargar el dataset
fifa_raw <- read_csv("C:/Users/Andres Schafler/Downloads/data.csv")
...
# 8. Comparación de Jugadores: Famoso vs Promedio

En esta sección, realizaremos un ejercicio práctico comparando las predicciones de valor de mercado para dos perfiles de jugadores muy distintos:
1.  **Jugador Famoso:** Un jugador de élite mundial (ej. Kylian Mbappé o Lionel Messi).
2.  **Jugador Promedio:** Un jugador que represente las características medias de la muestra.

Utilizaremos dos modelos para estas predicciones:
1.  **OLS Completo:** El modelo de regresión lineal múltiple más completo que hemos construido (incluyendo posición y pie dominante).
2.  **GLM Gamma:** El modelo lineal generalizado que ajustamos anteriormente.

## 8.1 Definición del Modelo OLS Completo

```{r modelo_ols_completo}
# Ajustar el modelo OLS más completo con todas las variables disponibles
modelo_ols_completo <- lm(log_value_eur ~ overall + potential + age + 
                            international_reputation + preferred_foot + posicion_grupo,
                          data = fifa)

summary(modelo_ols_completo)
```

## 8.2 Selección de Jugadores

```{r seleccion_jugadores}
# 1. Jugador Famoso: Buscamos a Kylian Mbappé
# Nota: Usamos str_detect para encontrarlo por si el nombre exacto varía
jugador_famoso <- fifa %>% 
  filter(str_detect(`Known As`, "Mbappé")) %>% 
  arrange(desc(overall)) %>% 
  head(1)

# 2. Jugador Promedio: Buscamos un jugador con características cercanas a la media
media_overall <- mean(fifa$overall, na.rm = TRUE)
media_age <- mean(fifa$age, na.rm = TRUE)

# Seleccionamos un jugador que esté cerca de la media en overall y edad
set.seed(123) # Para reproducibilidad
jugador_promedio <- fifa %>% 
  filter(
    overall >= media_overall - 2 & overall <= media_overall + 2,
    age >= media_age - 2 & age <= media_age + 2
  ) %>% 
  sample_n(1)

# Mostrar los jugadores seleccionados
jugadores_comparacion <- bind_rows(jugador_famoso, jugador_promedio) %>% 
  select(`Known As`, overall, potential, age, international_reputation, 
         preferred_foot, posicion_grupo, value_eur)

kable(jugadores_comparacion, caption = "Perfiles de los Jugadores Seleccionados")
```

## 8.3 Predicción y Comparación

```{r prediccion_comparacion}
# Realizar predicciones
predicciones <- jugadores_comparacion %>% 
  mutate(
    # Predicción OLS Completo (recordar hacer exp() para volver a euros)
    pred_ols_completo = exp(predict(modelo_ols_completo, newdata = .)),
    
    # Predicción GLM Gamma (predicción directa)
    pred_glm = predict(modelo_glm, newdata = ., type = "response")
  ) %>% 
  select(`Known As`, value_eur, pred_ols_completo, pred_glm) %>% 
  mutate(
    # Formatear para mostrar en millones
    Valor_Real_M = scales::dollar(value_eur, scale = 1e-6, suffix = "M€", prefix = "€"),
    Pred_OLS_M = scales::dollar(pred_ols_completo, scale = 1e-6, suffix = "M€", prefix = "€"),
    Pred_GLM_M = scales::dollar(pred_glm, scale = 1e-6, suffix = "M€", prefix = "€"),
    # Calcular error porcentual
    Error_OLS_Pct = scales::percent((pred_ols_completo - value_eur) / value_eur, accuracy = 0.1),
    Error_GLM_Pct = scales::percent((pred_glm - value_eur) / value_eur, accuracy = 0.1)
  )

kable(predicciones %>% select(`Known As`, Valor_Real_M, Pred_OLS_M, Error_OLS_Pct, Pred_GLM_M, Error_GLM_Pct),
      caption = "Comparación de Valor Real vs Predicho")
```
...
**Análisis del Jugador Famoso (`r jugador_famoso[["Known As"]]`):**
Observamos un fenómeno interesante: nuestros modelos **sobreestiman** significativamente el valor de Kylian Mbappé (prediciendo >300M€ vs real ~190M€). Esto sugiere que:
1.  **Techo de Mercado:** El modelo asume un crecimiento exponencial continuo, pero en la realidad existe un "techo" financiero que los clubes pueden pagar.
2.  **Rendimientos Decrecientes:** A niveles extremos de habilidad (Overall > 90), el valor marginal de cada punto extra podría ser menor de lo que predice una función exponencial simple.
3.  **Límites del Modelo:** La función exponencial crece muy rápido en los extremos, "disparando" la predicción más allá de lo razonable en el mercado actual.

**Análisis del Jugador Promedio (`r jugador_promedio[["Known As"]]`):**
Para el jugador promedio, las predicciones son mucho más precisas, con errores porcentuales bajos (en el rango de +/- 10%). Esto confirma que nuestros modelos son excelentes para explicar la estructura general del mercado, donde se encuentra la gran mayoría de los datos.

**Conclusión de la Comparación:**
Este resultado ilustra el peligro de la **extrapolación**: nuestros modelos funcionan bien en el rango donde hay muchos datos (jugadores promedio), pero pueden comportarse de manera impredecible o poco realista en los extremos (superestrellas), donde la dinámica de precios obedece a restricciones financieras no capturadas por las variables técnicas.

---

# 9. Conclusiones y Resumen del Análisis
